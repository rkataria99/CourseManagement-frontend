...
function InstanceList({ refresh }) {
  ...
  const fetchInstances = () => {
    ...
    axios
      .get(`https://coursemanagement-backend-sfnq.onrender.com/api/instances/${year}/${semester}`)
      .then(res => {
        const data = res.data;
        if (data.length === 0) {
          setNotFound(true);
  ...
  const handleDelete = (courseId) => {
    axios
      .delete(`https://coursemanagement-backend-sfnq.onrender.com/api/instances/${year}/${semester}/${courseId}`)
      .then(fetchInstances)
